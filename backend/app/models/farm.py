...
class Farm(Base):
    ...
    # Administrative-division reference (see app/models/location.py). All
    # nullable and independent of each other - a farmer may pick only as
    # far down the hierarchy as they know/the master data supports (e.g.
    # state+district with no mandal/village yet, since those tables have
    # no seed data). Consistency across the chain (district belongs to
    # state, etc.) is enforced at the service layer, not by the schema.
    ...
    mandal_id: Mapped[int | None] = mapped_column(
        Integer, ForeignKey("mandals.id", ondelete="SET NULL"), nullable=True, index=True
    )
    village_id: Mapped[int | None] = mapped_column(
        Integer, ForeignKey("villages.id", ondelete="SET NULL"), nullable=True, index=True
    )
    ...
    # Read-only lookups for display (e.g. "Guntur district") - never
    # cascade-deleted from here; the FK's ondelete=SET NULL already
    # handles a removed master-data row.
    state: Mapped["State | None"] = relationship(viewonly=True)
    district: Mapped["District | None"] = relationship(viewonly=True)
    mandal: Mapped["Mandal | None"] = relationship(viewonly=True)
    village: Mapped["Village | None"] = relationship(viewonly=True)
