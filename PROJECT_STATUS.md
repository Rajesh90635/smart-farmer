...
## Phase 41 follow-up part 5: live end-to-end verification of the location dropdowns

**Requested directly by the user** ("run the flutter app and check the dropdowns work"), after three straight migrations to the `villages` table with no live UI check yet - `flutter analyze`/`flutter test` verify the Dart code compiles and unit-level logic is correct, not that a farmer looking at the real cascading State/District/Mandal/Village dropdowns actually sees the newly seeded data.

**Environment note, discovered and worked around this pass**: no `chromium-cli` was available in this environment (unlike the tool this project's `run` skill recommends by default). Installed Playwright directly (`npm install playwright` + `npx playwright install chromium` in a scratch directory) as the fallback the skill itself documents - this worked cleanly and is a reusable pattern for future live-verification passes here.

**A real Flutter-web-specific mechanic, not obvious from the DOM alone**: a `flutter build web` release build renders everything through CanvasKit onto a `<canvas>` - there are no real interactive DOM elements for Playwright's text/role selectors to find until Flutter's own accessibility layer is switched on. The page ships a 1x1 off-screen `<flt-semantics-placeholder role="button" aria-label="Enable accessibility">` for screen readers; clicking it (via a native `element.click()`, since it's outside the viewport and Playwright's normal click refuses) populates a real `<flt-semantics>` DOM tree with `role`/`aria-label`/tappable nodes matching on-screen text - after that, `page.getByText(...)` works normally. Dropdown/menu **overlays** turned out to render outside whatever `document.body.innerText` captures (a separate overlay layer, not reflected in the semantics tree the same way) - those were driven with coordinate-based clicks against a fixed 1280x900 viewport instead, verified stable across repeated runs.

**Full flow actually driven, backend + release web build both real** (`uvicorn` on the dev database with all three new village migrations applied, `flutter build web --dart-define=API_BASE_URL=http://127.0.0.1:8000/api/v1` served via a local static server): registered a fresh farmer end-to-end (Welcome -> Create account -> language selection -> consent) to a real Home screen, navigated My Farms -> Add Farm, and drove the location section for real:
- **State**: opens to the real "Andhra Pradesh" row from the API.
- **District**: all 26 real districts, alphabetically listed.
- **Mandal** (District = Guntur): all 18 real Guntur mandals, including both "Chebrolu" (has real village data) and "Guntur East"/"Guntur West" (the deliberately village-less structural-split gap disclosed in the earlier phases) - both cases render correctly, no crash or broken state on the empty-village mandal.
- **Village** (Mandal = Chebrolu): real seeded villages appear - Chebrole, Godavarru, Manchala, Meesaragadda Ananthavaram, Narakodur, Pathareddipalem, Sekuru, Sreerangapuram, Suddapalli, Vadlamudi, Vejendla.
...
## Phase 41 follow-up part 6: 26 more mandals recovered via cross-district search

**Requested directly by the user** ("try to recover more of the 49 unmatched mandals"). Two avenues were tried before touching any matching code:

**Tried and rejected**: whether the single official mandal behind each Rural/Urban/East/West/North/South structural split (Anantapur, Chittoor, Guntur, Kurnool, Nandyal, Ongole, Vizianagaram, Adoni, Machilipatnam) tags its own villages with a (Rural)/(Urban)/(R)/(U) qualifier that could drive a correct split. Checked by actually fetching each one's real village list: the tags exist but only on a handful of villages each (Anantapur's 21 villages: 3 tagged "(RURAL)", 1 "(Ct)", 17 untagged) - nowhere near a clean, source-backed way to split all of them. Correctly left alone.

**Tried and it worked**: searching every official mandal across all 26 districts (not just the one AP CODES itself files it under) for a name match to each of the 49 unmatched mandals. This surfaced a genuine, previously-undiscovered pattern: several mandals are filed by AP CODES under a DIFFERENT district than Wikipedia (used for `c1a2b3d4e5f6`) assigned them to - e.g. Annamayya/Chowdepalle is an exact-name match to AP CODES' own Chittoor/Chowdepalle record; nine Prakasam mandals match records AP CODES files under Bapatla or SPSR Nellore; four Tirupati mandals match records filed under Annamayya. This is a real cross-source district-boundary disagreement, not a reason to change this table's own `mandals.district_id` (out of scope, left exactly as-is) - a mandal's real village list doesn't depend on which district bucket either source files it under.

**Migration `0106c9d5f59f`**: 26 mandals recovered - 21 via the cross-district search (some exact, a few near-exact spelling variants like "Ballikuruva"→"Ballikurava"), 4 via individually Telugu-name-verified pairing rather than blind distance matching (Siddavatam="Sidhout", Sri Avadhutha Kasinayana="S.A.K.N. MANDAL", Beerangi Kothakota="B Kothakota", Rajamahendravaram Rural/Urban="Rajahmundry Rural/Urban", Kadapa="Cuddapah"), and 1 same-district match the first pass's threshold had simply missed (Kukunuru↔"Kukunoor", distance 2). 450 real village rows added (15,988 -> 16,438).

**Two source-data quality artifacts found and disclosed, not silently resolved**: AP CODES' own mandal list has an ambiguous duplicate code (5148) shared between "Kandukur" and one of two "Voletivaripalem" entries - resolved by fetching both candidate codes directly and confirming they return two genuinely different, non-overlapping village lists before assigning each to the correct mandal. Separately, "Rajamahendravaram Urban"'s one village is literally spelled "Rajajmundry Urban" in the source (a typo for "Rajahmundry Urban") - preserved verbatim rather than silently corrected.

**Verified the same way as every prior migration**: upgrade (16,438, confirmed per-mandal counts match exactly) -> downgrade (back to 15,988) -> re-upgrade (16,438 again) -> re-run upgrade (idempotent) -> `alembic revision --autogenerate` diff empty. Applied to the dev database; `tests/test_location.py` (8/8) re-confirmed against the separate test database.

**Remaining, correctly disclosed and unchanged**: 23 mandals are still village-less - the genuine structural splits (22, unchanged from before) plus Alluri Sitharama Raju's Etapaka/Gurthedu and Annamayya's Vayalpad, whose nearest official candidates were manually reviewed and rejected as genuinely different real places, not this table's mandal under another name.


---

## My Estimates screen: a real gap closed, not a redesign

**Requested directly by the user**, after a live walkthrough of the four crop-financial screens (Ledger, Financial Summary, Input Spend Breakdown, Profit Forecast) surfaced a genuine, narrow gap: `Financial Summary`'s single `Estimated Cost` figure is a real backend SUM over every `CropCostEstimate` row for the crop cycle - entering "seed: 500" and later "seed: 700" doesn't overwrite the first estimate, it adds a second row and the total silently becomes 1200. Confirmed live before writing any code. `CropFinancialRepository.listEstimates()`/`deleteEstimate()` already called the real, already-working backend endpoints (`GET`/`DELETE /cost-estimates`) but were never invoked from any screen - grepped the full `mobile/lib` tree to confirm zero call sites.

**New screen**: `CropCostEstimatesScreen` (`features/crop_financial/crop_cost_estimates_screen.dart`), reached via a new list icon in Financial Summary's app bar (mirroring the Ledger screen's own "Invoices" shortcut pattern exactly, not a new UI convention). Lists every individual estimate (category, amount, optional description) with a delete button per row, plus its own "+" to add one - deliberately a self-contained copy of the existing add-estimate bottom sheet rather than a shared abstraction, matching how Ledger and Financial Summary each already keep their own near-identical entry sheet rather than being forced into one shared widget for two call sites. Returning from this screen refreshes Financial Summary, so the totals reflect any add/delete immediately.

**No backend or repository change** - `listEstimates`/`deleteEstimate` already existed and already worked; this phase only gave them a UI. Also confirmed and left correctly alone: stage-wise cost estimates (`crop_stage_definition_id`) are plumbed through the repository call but no screen anywhere - not this one, not Ledger's entry sheet - actually exposes a stage picker, so `Financial Summary`'s "Stage-wise Breakdown" section will keep rendering empty until a future phase adds that picker. Not attempted here; disclosed, not silently left unmentioned.

**Verified live in a real browser** (backend + rebuilt release web build, Playwright-driven): added a `seed: 500.00` estimate from the existing Financial Summary sheet, opened the new "My Estimates" screen and confirmed the real card rendered, deleted it, confirmed the list correctly showed "No estimates yet" and - critically - that Financial Summary's `Estimated Cost`/`Cost Variance` correctly reverted to `Not available` (never a fabricated `0`), proving the delete reached the real backend and both screens read the same live data.

**`flutter analyze`**: 34 issues (33 baseline + 1 new - the same already-established `DropdownButtonFormField` `value:` deprecation used by every other entry-form sheet in this app, not a new issue class). **`flutter test`: 226/226 passed**, zero regressions - no new widget test added, matching the existing precedent that neither Ledger, Financial Summary, nor Profit Forecast has one either (only their Dart models are unit-tested). Two new localization keys added to `app_en.arb` only (`myEstimatesTitle`, `noEstimatesYet`), English-only per this project's standing no-fabricated-translations rule; five more reused verbatim from the existing Financial Summary sheet.
